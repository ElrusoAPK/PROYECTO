...
function cargarAnios() {
    ...
    datosOriginales.forEach(d => {
        if (!d.Fecha) return;
        anios.add(d.Fecha.substring(0, 4));
    });
...
document.getElementById("filtroMagnitud")
    .addEventListener("change", aplicarFiltros);
...
function aplicarFiltros() {
    ...
    datosFiltrados = datosOriginales.filter(d => {
        ...
        if (anio) {
            cumpleAnio = d.Fecha && d.Fecha.startsWith(anio);
        }

        if (magnitud) {
            cumpleMagnitud = parseFloat(d.Magnitud) >= parseFloat(magnitud);
        }
...
function procesarDatos(datos) {
    ...
    document.getElementById("resumen").innerHTML = `
        <strong>${datos.length.toLocaleString()}</strong> eventos sísmicos analizados.
        <br><br>
